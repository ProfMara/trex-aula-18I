...
function setup() {
    ...
    //solo invisível
    soloInvisivel = createSprite(width/2,height-5,width,20);
    soloInvisivel.visible = false;

    //trex 
    trex = createSprite(width/15,height-30,50,50);
...
function draw() {
    ...
    if(gameState == PLAY){
        ...
        fill ("black");
        textSize (25);
        text (score, width*0.88, height/8);
...
function criarNuvens(){
    ...
    if(frameCount % 90 == 0){
        ...
        var nuvem = createSprite(width,Math.round(random(height/15,height*0.66)),75,20);
        //adiciona a imagem
...
function criarCactos(){
    if(frameCount % 60 == 0){
        var cacto = createSprite(width,height-30,50,50);
